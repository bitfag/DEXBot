...
class Strategy(StrategyBase):
    ...
    def increase_order_sizes(self, asset, asset_balance, orders):
        ...
        if (self.mode == 'mountain' or
            ...
            for order in orders:
                ...
                if (further_bound > order_amount * (1 + self.increment / 10) < closer_bound and
                    ...
                    if not self.mountain_max_increase_mode:
                        # 1. Increase furthest for at least self.min_increase_factor (avoid too much orders
                        #    replacements)
                        # 2. Smooth increase for orders between furthest and closest (see docstring example)
                    ...
                    if is_least_order:
                        ...
                        increased_amount = order_amount * (total_balance / new_orders_sum) * (1 + self.increment)
            ...
            for order in orders:
                ...
                if (order_amount_normalized < further_order_bound and
                    ...
                    if new_order_amount < order_amount_normalized:
                        ...
                        order_amount_normalized < closest_order_bound and
            ...
            closest_order = orders[-1]
            increase_factor = max(1 + self.increment, self.min_increase_factor)
            initial_closest_order_bound = closest_order['base']['amount'] * (1 + self.increment)
            ...
            for order in orders:
                ...
                if order_index + 1 < orders_count:
                    ...
                    closest_order_bound = initial_closest_order_bound / (math.sqrt(1 + self.increment) ** \
                                          reverse_index)
                else:
                    is_closest_order = True
                    closer_order_bound = order['base']['amount'] * increase_factor
                ...
                if (order_amount_normalized < further_order_bound and
                    ...
                    if is_closest_order:
                        ...
                        # Then apply balance limit
                        new_order_amount = min(new_order_amount, asset_balance['amount'] + order['for_sale']['amount'])
                    else:
